...
namespace TestLibrary
{
    public class Maze
    {
        ...
        {
            string result = string.Empty;
            if (this.RunMazeBFS(this.startingRow, this.startingColumn))
            {
                result = String.Format("Path to follow from Start [{0}, {1}] to Exit [{2}, {3}] - {4} steps:\n", this.startingRow, this.startingColumn, endPoint.Row, endPoint.Column, path.Size);
                result += this.PrintPath();
                result += this.PrintMaze();
        ...
        private bool RunMazeBFS(int row, int column)
        {
            Queue<Point> queue = new Queue<Point>();
            Point startPoint = new Point(startingRow, startingColumn);

            // Enqueue the starting point to the queue
            ...
            while (!queue.IsEmpty())
            {
                ...
                // Out of bounds check
                if (x < 0 || y < 0 || x >= this.charMaze.Length || y >= this.charMaze[x].Length)
                    continue;

                // Get value from Maze
                char cell = this.charMaze[x][y];

                // Skip if it is wall or visited
                if (cell == 'W' || cell == 'V')
                    continue;
                ...
                if (cell == 'E')
                {
                    // Mark the path from the end to the start
                    while (current != null)
                    {
                        // push to path stack
                        path.Push(current);
                        
                        // Set "." to the path only if it's not the endpoint 'E' and set end point
                        if (this.charMaze[current.Row][current.Column] != 'E')
                            this.charMaze[current.Row][current.Column] = '.';
                        else
                            this.endPoint = new Point(current.Row, current.Column);
                ...
                south.ParentPoint = current;
                queue.Enqueue(south);

                // East
                Point east = new Point(x, y + 1);
                east.ParentPoint = current;
                queue.Enqueue(east);

                // West
                Point west = new Point(x, y - 1);
                west.ParentPoint = current;
                queue.Enqueue(west);

                // North
                Point north = new Point(x - 1, y);
                north.ParentPoint = current;
                queue.Enqueue(north);
            }
